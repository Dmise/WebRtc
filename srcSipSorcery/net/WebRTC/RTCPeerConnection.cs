...
namespace SIPSorcery.Net
{
    ...
    public class RTCPeerConnection : RTPSession, IRTCPeerConnection
    {
        ...
        private ILogger<RTCPeerConnection> _logger;
        /// <summary>
        ...
        public RTCPeerConnection(ILogger<RTCPeerConnection> logger, bool nothing) : this()
        {           
            _logger = logger;
        }
            ...
            base(true, true, true, configuration?.X_BindAddress, bindPort, portRange)
        {
            ...
            if (configuration != null)
            {
                ...
                {
                    _logger.WarnLine("No DTLS certificate is provided in the configuration");
                }
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if (iceState == RTCIceConnectionState.connected && _rtpIceChannel.NominatedEntry != null)
            {
                if (_dtlsHandle != null)
                {
                        ...
                        base.PrimaryStream.DestinationEndPoint?.Port != _rtpIceChannel.NominatedEntry.RemoteCandidate.DestinationEndPoint.Port)
                    {
                        ...
                        _logger.InfoLine($"ICE changing connected remote end point to {connectedEP}.");
                    }
                ...
                else
                {
                    ...
                    SetGlobalDestination(connectedEP, connectedEP);
                    _logger.InfoLine($"ICE connected to remote end point {connectedEP}.");
                    ...
                    _logger.DebugLine($"Starting DLS handshake with role {IceRole}.");
                    ...
                    {
                        _logger.WarnLine($"RTCPeerConnection DTLS handshake failed. {excp.Message}");
        ...
        public SetDescriptionResultEnum setRemoteDescription(RTCSessionDescriptionInit init)
        {
            ...
            switch (signalingState)
            {
                case var sigState when sigState == RTCSignalingState.have_local_offer && sdpType == SdpType.offer:
                    _logger.WarnLine($"RTCPeerConnection received an SDP offer but was already in {sigState} state. Remote offer rejected.");
            ...
            if (setResult == SetDescriptionResultEnum.OK)
            {
                ...
                foreach (var ann in remoteSdp.Media)
                {
                        ...
                        ann.ApplicationMediaFormats.Single().Key == SDP_DATACHANNEL_FORMAT_ID)
                    {
                        ...
                        {
                            _logger.WarnLine($"The remote SDP requested an unsupported data channel transport of {ann.Transport}.");
                ...
                if (!string.IsNullOrWhiteSpace(dtlsFingerprint))
                {
                    ...
                    {
                        _logger.WarnLine($"The DTLS fingerprint was invalid or not supported.");
                        return SetDescriptionResultEnum.DtlsFingerprintDigestNotSupported;
                    }
                }
                else
                {
                    _logger.WarnLine("The DTLS fingerprint was missing from the remote party's session description.");
                ...
                _logger.DebugLine($"SDP:[{remoteSdp}]");
        ...
        public override void Close(string reason)
        {
            if (!IsClosed)
            {
                _logger.LogDebug($"Peer connection closed with reason {(reason != null ? reason : "<none>")}.");
        ...
        private SDP createBaseSdp(List<MediaStream> mediaStreamList, bool excludeIceCandidates = false)
        {
            ...
            foreach (var mediaStream in mediaStreamList)
            {
                ...
                {
                    _logger.WarnLine($"Media announcement for {mediaStream.LocalTrack.Kind} omitted due to no reciprocal remote announcement.");
                }
                else
                {
            ...
            if (DataChannels.Count > 0 || (RemoteDescription?.Media.Any(x => x.Media == SDPMediaTypesEnum.application) ?? false))
            {
                ...
                if (mindex == SDP.MEDIA_INDEX_NOT_PRESENT)
                {
                    _logger.WarnLine($"Media announcement for data channel establishment omitted due to no reciprocal remote announcement.");
                }
                else
                {
            ...
            if (buffer?.Length > 0)
            {
                try
                {
                    ...
                    else
                    {
                        if (_dtlsHandle != null)
                        {
                            _logger.DebugLine($"DTLS transport received {buffer.Length} bytes from {AudioDestinationEndPoint}.");
                            _dtlsHandle.WriteToRecvStream(buffer);
                        }
                        else
                        {
                            _logger.WarnLine($"DTLS packet received {buffer.Length} bytes from {remoteEP} but no DTLS transport available.");
                        }
                    }
                }
                catch (Exception excp)
                {
                    _logger.WarnLine($"Exception RTCPeerConnection.OnRTPDataReceived {excp.Message}");
                }
            }
        }
        ...
        public void addIceCandidate(RTCIceCandidateInit candidateInit)
        {
            ...
            {
                _logger.WarnLine($"Remote ICE candidate not added as no available ICE session for component {candidate.component}.");
            }
        }
        ...
        private async Task InitialiseSctpTransport()
        {
            try
            {
                _logger.InfoLine(nameof(InitialiseSctpTransport));
            ...
            {
                _logger.ErrorLine($"SCTP exception establishing association, data channels will not be available. {excp}");
                sctp?.Close();
            }
        }
        ...
        private void OnSctpTransportStateChanged(RTCSctpTransportState state)
        {
            ...
            {
                _logger.DebugLine("SCTP transport successfully connected.");
        ...
        {
            _logger.InfoLine($"WebRTC new data channel opened by remote peer for stream ID {streamID}, type {type}, " +
            ...
            else
            {
                ...
                _logger.WarnLine($"WebRTC duplicate data channel requested for stream ID {streamID}.");
            }
        }
        ...
        private void OnSctpAssociationDataChannelOpened(ushort streamID)
        {
            ...
            _logger.InfoLine($"WebRTC data channel opened label {label} and stream ID {streamID}.");
            ...
            {
                _logger.WarnLine($"WebRTC data channel got ACK but data channel not found for stream ID {streamID}.");
            }
        }
        ...
        private void OnSctpAssociationDataChunk(SctpDataFrame frame)
        {
            ...
            {
                _logger.WarnLine($"WebRTC data channel got data but no channel found for stream ID {frame.StreamID}.");
            }
        }
        ...
        private async Task InitialiseSctpAssociation()
        {
            ...
            if (sctp.state != RTCSctpTransportState.Connected)
            {
                ...
                {
                    _logger.DebugLine($"SCTP transport for create data channel request changed to state {state}.");
        ...
        {
            _logger.DebugLine($"Data channel create request for label {label}.");
        ...
        private void OpenDataChannel(RTCDataChannel dataChannel)
        {
            if (dataChannel.negotiated) {
                _logger.DebugLine($"WebRTC data channel negotiated out of band with label {dataChannel.label} and stream ID {dataChannel.id}; invoking open event");
                dataChannel.GotAck();
            }
            else if (dataChannel.id.HasValue)
            {
                _logger.DebugLine($"WebRTC attempting to open data channel with label {dataChannel.label} and stream ID {dataChannel.id}.");
                dataChannel.SendDcepOpen();
            }
            else
            {
                _logger.ErrorLine("Attempt to open a data channel without an assigned ID has failed.");
            }
        }
        ...
        {
            _logger.DebugLine("RTCPeerConnection DoDtlsHandshake started.");
            ...
            if (!handshakeResult)
            {
                ...
                _logger.WarnLine($"RTCPeerConnection DTLS handshake failed with error {handshakeError}.");
                Close("dtls handshake failed");
                return false;
            }
            else
            {
                _logger.DebugLine($"RTCPeerConnection DTLS handshake result {handshakeResult}, is handshake complete {dtlsHandle.IsHandshakeComplete()}.");
                ...
                {
                    _logger.WarnLine($"RTCPeerConnection remote certificate fingerprint mismatch, expected {expectedFp}, actual {remoteFingerprint}.");
                    Close("dtls fingerprint mismatch");
                    return false;
                }
                else
                {
                    _logger.DebugLine($"RTCPeerConnection remote certificate fingerprint matched expected value of {remoteFingerprint.value} for {remoteFingerprint.algorithm}.");
        ...
        private void OnDtlsAlert(AlertLevelsEnum alertLevel, AlertTypesEnum alertType, string alertDescription)
        {
            ...
            {
                _logger.DebugLine($"SCTP closing transport as a result of DTLS close notification.");
            ...
            else
            {
                ...
                _logger.WarnLine($"DTLS unexpected {alertLevel} alert {alertType}{alertMsg}");
            }
        }
